...
$salary_form = get_post_meta($job_id, 'salary_form', true);
$salary_min = get_post_meta($job_id, 'salary_min', true);
$salary_max = get_post_meta($job_id, 'salary_max', true);
$fixed_salary = get_post_meta($job_id, 'fixed_salary', true);
$salary_remarks = get_post_meta($job_id, 'salary_remarks', true);
...
if (isset($_POST['working_hours'])) {
    update_post_meta($job_id, 'working_hours', sanitize_textarea_field($_POST['working_hours']));
}

if (isset($_POST['holidays'])) {
    update_post_meta($job_id, 'holidays', sanitize_textarea_field($_POST['holidays']));
}
...
<div class="edit-job-container">
    ...
    <span class="form-hint">例: 9:00〜18:00（休憩60分）<br>複数の勤務時間がある場合は改行して入力してください</span>
</div>
...
<div class="form-row">
    ...
    <span class="form-hint">例: 土日祝、年末年始、有給休暇あり<br>複数の休日・休暇制度がある場合は改行して入力してください</span>
</div>
